...
3. **CLI line-up** (week 2–3) — per-brain branded CLIs, same Python source, different binary name + default endpoint baked at provision time. `nous` (public, `pip install nous`, talks to nous.brainfoundry.ai) is the canonical public test target. Branded per-owner CLIs (e.g. `alice`, `bob`) are generated by the provisioner at buy-time. Verbs on every binary: `<name> chat`, `<name> upload --layer X`, `<name> stats`, `<name> forget-doc`, `<name> --init`.
