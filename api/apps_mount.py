...
def mount_app(app: FastAPI, entry: dict[str, Any]) -> None:
    """Mount a single app's UI bundle and optional API router. Idempotent only
    in the sense that re-calling appends another route — FastAPI does not
    ...
    if ui_bundle:
        ...
        elif bundle_dir.exists() and (bundle_dir / "index.html").exists():
            app.mount(
                f"/apps/{app_id}",
