...
| Saved memories, notes, future email | **untrusted** | route through the wrapper |
| Tool *names/args* proposed by the model | **untrusted** | fail-closed gate (`api/tools.is_blocked_tool`) |
...
5. **No SSRF guard on caller-supplied endpoints.** Today no endpoint accepts a
   caller-supplied `base_url`. If one is ever added, it must validate
   scheme + resolved address against an allowlist and block link-local / metadata
